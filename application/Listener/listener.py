...
while 1:  # Run forever
    new_hash = gethash()
    print("Initial Hash " + current_hash)
    print("Fetched fresh " + new_hash)
    if new_hash == current_hash:  # If nothing has changed
        print("Not Changed")
    else:  # If something has changed
        print("Changed")
        print("Trigger webscraping logic - request to Admin")
        print("After webscrapping, Save the new hash in DB " + new_hash)
